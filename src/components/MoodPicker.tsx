...
export function MoodPicker({ onMoodSelect, activeMood }: MoodPickerProps) {
  return (
    <section className="bg-zinc-900/30 border border-white/5 rounded-[3rem] p-12 space-y-8">
      <div className="flex items-center justify-between">
        <div className="space-y-1">
          <h3 className="text-3xl font-black text-white uppercase tracking-tight font-display">What's your mood tonight?</h3>
          <p className="text-xs font-bold text-gray-500 uppercase tracking-[0.2em]">Pick a vibe, we'll find the perfect watch</p>
        </div>
        {activeMood && (
          <button 
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        {MOOD_MAPPINGS.map((mood) => (
          <motion.button
            ...
            className={cn(
              "flex flex-col items-center justify-center gap-4 p-8 rounded-[2rem] border transition-all duration-500 group relative overflow-hidden",
